...
contract SendEthfrmCont {
    ...
    function sendEther(address payable recipient, uint256 amt) external {
        ...
        // here, gas limit can be set to any value. We are setting it to 2300 as used by `transfer`, `send` functions.
        // for small operation like this, we don't need much gas cost than 2300. But for complex operations, we might
        // have to set higher values. So, `.call` allows to set that required value.
        // you should analyze the specific operations being executed and determine the minimum gas required for those operations.
        (bool success,) = recipient.call{gas: 2300, value: amt}(new bytes(0));
        require(success, "Transfer failed.");
