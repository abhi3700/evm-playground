...
const UPGRADEABLE_TOKEN_CONTRACT =
  "0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0" || ""; // tested in localhost
...
async function main(): Promise<void> {
  ...
  const token: Contract = await upgrades.upgradeProxy(
    UPGRADEABLE_TOKEN_CONTRACT,
    TokenFactory
  );

  console.log(`Token contract upgraded to: ${token.address}`);
  console.log(
    `The transaction that was sent to the network to upgrade the deployed token contract: ${token.deployTransaction.hash}`
  );
}
